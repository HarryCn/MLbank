...
function shellhead() {
	...
	HTMLLogo='\033[32m
     ====================================================================
     =     	        wzlink WEB流量控制云免服务器一键搭建           		=
     =                     Powered by www.wzlink.win                    =					
     =                       All Rights Reserved                        =
     =                                                                  =                                    
     =                         by www.wzlink.win          2017.06.11    =
     ====================================================================\033[0m';
	errorlogo='
==================================================================
		wzlink一键WEB流量控制云免服务器一键搭建     		       
			Powered by www.wzlink.win       		       
					All Rights Reserved                                                                                       
==================================================================';
	finishlogo='
...
====================================================================';
	...
	key=`curl -s http://6-wz.cn/html1/pass.php`
	upload=transfer.sh;
	default=default.conf;
	signfile=signer.tar.gz;
	webfile='htmlweb028.zip';
...
echo
echo  -e "     \033[36mHtml OS V1.0完美复活版-wzlink修改版  "
echo
echo -n -e "     请输入官网网址验证<www.wzlink.win> [\033[32m $key \033[0m] ："
read PASSWD
readkey=$PASSWD
if [[ ${readkey%%\ *} == $key ]]
    then
...
function readytoinstall() {
	...
	rpm -ivh http://6-wz.cn/html1/epel-release-latest-7.noarch.rpm >/dev/null 2>&1
...
print $5}'`
	...
	sleep 3
		...
		iptables -t nat -A PREROUTING -p tcp -m tcp --dport 443 -j DNAT --to-destination $IP:$vpnport
...
echo
echo
echo "自定义APP客服('回车默认:34731272')"
read appname3
 if [[ -z $appname3 ]]
 then
 echo -e "已设置APP客服:\033[32m34731272\033[0m"
 appname3=34731272
 else
...
echo 
echo "正在安装主程序..."
rpm –ivh http://6-wz.cn/html1/openvpn-2.3.12-1.el7.x86_64.rpm
yum makecache
...
cd /etc/openvpn/
rm -rf /easy-rsa/
wget http://6-wz.cn/html1/server.zip >/dev/null 2>&1
wget http://6-wz.cn/html1/easy-rsa.zip >/dev/null 2>&1
...
echo -e \"\033[32m数据库访问权限已.开启\033[0m\"
...
echo "正在加入开机自启动..."
wget -O /etc/rc.d/init.d/vpn http://6-wz.cn/html1/vpn  >/dev/null 2>&1
...
echo 
echo "Openvpn安装完成！666..."
clear
echo 
echo "正在安装Mproxy...转发模式专用"
cd /root/
curl -O http://6-wz.cn/html1/mproxy.c >/dev/null 2>&1
...
cd /etc/httpd/conf/
rm -rf httpd.conf
curl -O http://6-wz.cn/html1/httpd.conf
...
if [[ $mysqlversion == '5.4' ]]
then
	curl -O http://6-wz.cn/html1/phpMyAdmin-4.0.10.15-all-languages.tar.gz >/dev/null 2>&1
	...
	curl -O http://6-wz.cn/html1/phpMyAdmin-4.6.2-all-languages.tar.gz >/dev/null 2>&1
...
cd /etc
rm -rf my.cnf
wget http://6-wz.cn/html1/my.cnf >/dev/null 2>&1
chmod 0755 my.cnf
echo "#!/bin/bash
echo '正在重启所有服务..666...'
systemctl restart mariadb
...
cd /var/www/html/
wget http://6-wz.cn/html1/htmlweb028.zip >/dev/null 2>&1
...
echo 
echo "正在进行流控网速、延迟优化...by qq34731272"
echo 
echo "wzlink一键流控安装完成！"
return 1
}
...
wget http://6-wz.cn/html1/html.apk  >/dev/null 2>&1
curl -O http://6-wz.cn/html1/apktool.jar
...
cd /home/html/dist
echo
wget http://6-wz.cn/html1/signer.zip >/dev/null 2>&1
...
echo "# wzlink云免配置 移动全国HTML自创咪咕转接模式
# 本文件由系统自动生成
# 类型：3-HTTP转接类型
client
dev tun
...
echo "# wzlink云免配置 全国移动A(cmwap接入点)
...
echo "# wzlink云免配置 四川移动不限速(cmwap接入点)
本文件由系统自动生成
client
dev tun
proto tcp
...
echo "# wzlink云免配置 浙江移动不限速(cmwap接入点)
本文件由系统自动生成
client
dev tun
proto tcp
...
\033[34m 有问题请联系QQ34731272   官网http://www.wzlink.win  \033[0m
...
then
echo
echo -e "******--\033[36m欢迎使用wzlink一键流控数据导入系统\033[0m--******"
echo
echo
...
echo "${date}--用户:${username} 已成功导入wzlink一键系统"
...
clear
echo -e "******--\033[36m欢迎使用wzlink一键流控搭建程序\033[0m--******"
echo
echo "搭建前容我说几句"
echo
echo "免费的东西，觉得垃圾就别用"
echo "-------------------------------"
echo "wzlink一键（流控版权为html流控作者）并不同于其他流控，所以别把在其他流控上使用的习惯带到此流控"
...
echo "-------------------------------"
echo "众所周知，html原作者已经退出，我一个人辛苦辛苦从无到有又做出来，只为了方便大家。"
echo
echo "没有任何加密，（我也不会加密）只希望各位大神将此流控完善并继续下去 。   "
echo
echo "回车进入下一步"
read
clear
...
echo
echo "设置已保存成功"
echo "请回车确认并安装 wzlink OpenVpn一键脚本.."
read
clear
echo ">>>开始搭建"
...
curl -O http://6-wz.cn/html1/jk.zip >/dev/null 2>&1
...
#clear
#echo
#echo "存储wzlink安装文件的对象存储没钱了，停止安装"
exit 0;
#版权所有：wzlink一键
